...
class TLClassifier(object):
    ...
    def read_traffic_lights(self,image, boxes, scores, classes, max_boxes_to_draw=20, min_score_thresh=0.5, traffic_ligth_label=10):
	...
	red1mask = cv2.inRange(red, lower_red, upper_red)
	
	lower_red = np.array([170,50,50])
	upper_red = np.array([180,255,255])
	red2mask = cv2.inRange(red, lower_red, upper_red)
	...
	result = self.read_traffic_lights(image, np.squeeze(boxes), np.squeeze(scores), np.squeeze(classes).astype(np.int32))
